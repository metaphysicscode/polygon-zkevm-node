...
func (a *Aggregator) sendGenerateFinalProof() {
	...
	for {
		...
		log.Debugf("sendGenerateFinalProof. monitoredTxID: %s, lastVerifiedBatchNum: %d", monitoredTxID, lastVerifiedBatchNum)
		a.finalProof <- msg
		lastVerifiedBatchNum = sequence.ToBatchNumber
	}
}
...
func (a *Aggregator) resendProofHash() {
	...
	for {
		...
		a.proofHashCommitEpoch = proofHashCommitEpoch
		a.proofCommitEpoch = proofCommitEpoch
		...
		for {
			...
			if lastBatchNum > 0 && lastBatchNum > sequence.ToBatchNumber {
				if curBlockNumber-initBlockNumber <= commitEpoch {
					break
				}
			}
...
func (a *Aggregator) sendFinalProof() {
	...
	for {
		...
		case <-tick.C:
			if len(a.txs) > 0 {
				log.Debugf("wait send proof tx. txs size: %d", len(a.txs))
				continue
			}
			lastVerifiedEthBatchNum, err := a.Ethman.GetLatestVerifiedBatchNum()
			if err != nil {
			...
			log.Infof("finalProofMsgs size: %d", len(finalProofMsgs))
			for i := 0; i < len(finalProofMsgs); i++ {
				log.Debugf("finalProofMsgs. index: %d, BatchNumber:%d, BatchNumberFinal: %d", i, finalProofMsgs[i].recursiveProof.BatchNumber, finalProofMsgs[i].recursiveProof.BatchNumberFinal)
			}
		...
		case proofHash := <-a.proofHashCH:
			...
			log := log.WithFields("batches", fmt.Sprintf("%d-%d", proverProof.InitNumBatch, proverProof.FinalNewBatch))
...
func (a *Aggregator) monitorSendProof(batchNumber, batchNumberFinal uint64, monitoredTxID string) {
	...
	for {
		...
		case <-a.ctx.Done():
			return
		case <-tick.C:
			log := log.WithFields("monitorSendProof monitoredTxID", monitoredTxID)
			if _, ok := a.txs[monitoredTxID]; ok {
				return
			}
			...
			log.Infof("proofHashBlockNum = %d, max_commit_proof = %d, blockNumber =%d, monitoredTxID = %s", proofHashBlockNum, a.proofHashCommitEpoch, blockNumber, monitoredTxID)
			if proofHashBlockNum == 0 || (proofHashBlockNum+uint64(a.proofHashCommitEpoch)) > blockNumber {
				continue
			}

			hash, err := a.State.GetProofHashBySender(a.ctx, a.cfg.SenderAddress, batchNumberFinal, uint64(a.proofHashCommitEpoch), blockNumber, nil)
			if err != nil {
	...
	if proof == nil {
		...
		if errFinalProof == nil {
			...
			if errors.Is(err, state.ErrNotFound) {
				...
				log.Debugf("No proof ready to verify. batchNum: %d", lastVerifiedBatchNum+1)
				return false, nil
			}
...
func (a *Aggregator) canVerifyProofHash() bool {
	a.TimeSendFinalProofHashMutex.RLock()
	defer a.TimeSendFinalProofHashMutex.RUnlock()
	return a.TimeSendFinalProofHash.Before(time.Now()) && !a.verifyingProofHash
}

func (a *Aggregator) startProofHash() {
	a.TimeSendFinalProofHashMutex.Lock()
	defer a.TimeSendFinalProofHashMutex.Unlock()
	a.verifyingProofHash = true
}

// endProofVerification set verifyingProof to false to indicate that there is not proof verification in progress
func (a *Aggregator) endProofHash() {
	a.TimeSendFinalProofHashMutex.Lock()
	defer a.TimeSendFinalProofHashMutex.Unlock()
	a.verifyingProofHash = false
}

// resetVerifyProofTime updates the timeout to verify a proof.
func (a *Aggregator) resetVerifyProofHashTime() {
	a.TimeSendFinalProofHashMutex.Lock()
	defer a.TimeSendFinalProofHashMutex.Unlock()
	a.TimeSendFinalProofHash = time.Now().Add(a.cfg.VerifyProofInterval.Duration)
}
