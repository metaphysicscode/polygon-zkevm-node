...
func NewSimulatedEtherman(cfg Config, auth *bind.TransactOpts) (etherman *Client, ethBackend *backends.SimulatedBackend, maticAddr common.Address, br *polygonzkevmbridge.Polygonzkevmbridge, err error) {
	...
	poeAddr, _, poe, err := polygonzkevm.DeployPolygonzkevm(auth, client, exitManagerAddr, rollupVerifierAddr, bridgeAddr, 1000, 1) //nolint
	...
	poeParams := polygonzkevm.PolygonZkEVMV2InitializePackedParameters{
		Admin:                    auth.From,
